...
19. DOCUMENT TOTALS (CRITICAL FOR VALIDATION): If the document shows a grand total value (e.g., "Total Value: $14,953,761.34"), extract it into document_totals.total_value. Similarly extract total day change if shown. This field is REQUIRED for multi-account documents — it is used to validate that your extracted data is correct. The sum of all position market values and the sum of all account balances MUST approximately equal this document total. If your extracted positions or balances don't add up to the document total, you have made an extraction error — go back and check your work.

20. SELF-VALIDATION (CRITICAL): Before returning your JSON, verify these internal consistency checks:
  a) The sum of all account balance liquidation_values should approximately equal document_totals.total_value (if both are present). If they differ by more than 5%, re-check your extraction.
  b) For accounts with positions, the sum of position market_values + cash_balance should approximately equal the account's balance liquidation_value. If they differ significantly, you likely misread a quantity, price, or market_value.
  c) For aggregate/unallocated positions, their total market value should be consistent with the investment account balances — NOT wildly larger. If aggregate positions total $5M but account balances total $50K, you have misread the document.
  d) Check that share quantities and market values are plausible. A position worth more than the document's stated total portfolio value is almost certainly wrong.
  e) Do NOT confuse columns when reading tables. Double-check that you're reading the correct column for quantity vs. price vs. market value. A common error is reading a dollar amount as a share quantity.

21. ASSET TYPE CLASSIFICATION: You MUST set asset_type on every position and transaction. Never leave it null, and never lazily default everything to "EQUITY". Use your knowledge of financial instruments and any context from the document (section headers, descriptions, ticker symbols) to classify each item into the correct type: "EQUITY", "ETF", "OPTION", "MUTUAL_FUND", "FIXED_INCOME", "CASH_EQUIVALENT", "REAL_ESTATE", "PRECIOUS_METAL", "COLLECTIBLE", or "OTHER_ASSET". If the document groups positions under section headers (e.g., "Equities", "ETFs & Closed End Funds"), those headers are the strongest signal for classification. For COLLECTIBLE and OTHER_ASSET, also set asset_subtype to a short descriptive label (e.g., "Jewelry", "Art", "Classic Car").`;
