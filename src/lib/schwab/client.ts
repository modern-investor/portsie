...
import { getSchwabCredentials } from "./credentials";
import type {
  SchwabTokenResponse,
  SchwabAccountNumber,
  SchwabAccount,
...
// --- OAuth helpers (instance-based, uses per-user credentials) ---
...
export class SchwabAuth {
  ...
  constructor(appKey: string, appSecret: string, callbackUrl?: string) {
    this.appKey = appKey;
  ...
  getAuthorizationUrl(state?: string): string {
    if (!this.callbackUrl) throw new Error("callbackUrl required for authorization");
  ...
  async exchangeCode(code: string): Promise<SchwabTokenResponse> {
    if (!this.callbackUrl) throw new Error("callbackUrl required for code exchange");
...
export function getCallbackUrl(requestOrigin?: string): string {
  if (process.env.SCHWAB_CALLBACK_URL) {
    return process.env.SCHWAB_CALLBACK_URL;
  }
  if (requestOrigin) {
    return `${requestOrigin}/schwab/callback`;
  }
  throw new Error("Cannot determine Schwab callback URL");
}
...
export async function getValidAccessToken(
  ...
  if (needsRefresh) {
    const creds = await getSchwabCredentials(supabase, userId);
    if (!creds) throw new Error("SCHWAB_CREDENTIALS_NOT_FOUND");
    const auth = new SchwabAuth(creds.appKey, creds.appSecret);
